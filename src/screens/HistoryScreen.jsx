...
export function HistoryScreen({ onBack, onOpen, theme = 'dark' }) {
  ...
  const accent = theme === 'light' ? '#c2410c' : 'var(--color-lime)';
      ...
      <div style={{ maxWidth: 640, margin: "0 auto" }}>
        <div className="fu" style={{ paddingTop: "2.5rem", paddingBottom: "2rem", display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: muted }}>{hist.length} saved tournaments</div>
          </div>
        </div>
        ...
        {hist.length === 0 ? (
          ...
          <div key={i} className="fu rh glass-card" onClick={() => onOpen(t)}
            ...
            <div style={{ color: muted }}><ChevronRight size={20} /></div>
          </div>
        ))}
      </div>
    </div>
              ...
              <div style={{ maxWidth: 800, margin: "0 auto" }}>
                {(t.champion || playoffs.champion) && (
                  <div className="fu glass-card" style={{ border: `1px solid var(--color-lime)`, borderRadius: 'var(--radius-lg)', padding: "2rem", textAlign: "center", marginBottom: 24, background: 'rgba(200, 241, 53, 0.05)', boxShadow: '0 8px 32px rgba(200, 241, 53, 0.1)' }}>
                    ...
                    <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 42, color: lime, letterSpacing: 3, textShadow: '0 0 20px rgba(200, 241, 53, 0.4)' }}>{t.champion || playoffs.champion}</div>
                  </div>
                )}
                {playoffs.isMini ? (
